...
class AITrader {
  constructor() {
    ...
    this.model = process.env.DEEPSEEK_MODEL || process.env.AI_MODEL || 'deepseek-v4-pro';
...
Analyze the market data and patterns provided. Return ONLY valid JSON with:
{
  ...
  prepareMarketData(coin, candles, indicators, patterns, signal) {
    ...
    const lastTimestamp = Number(candles[candles.length - 1]?.[0]) || Date.now();
    const targetTimestamp = lastTimestamp - 24 * 60 * 60 * 1000;
    let comparisonIndex = 0;
    for (let index = candles.length - 1; index >= 0; index--) {
      if ((Number(candles[index]?.[0]) || 0) <= targetTimestamp) {
        comparisonIndex = index;
        break;
      }
    }
    const comparisonPrice = closes[comparisonIndex] || closes[Math.max(0, closes.length - 2)] || lastPrice || 1;
    const priceChange = ((lastPrice - comparisonPrice) / comparisonPrice * 100) || 0;
